...
interface IUniswapV3LiquidityBytesTest {
    function revertSwapCallback(
        CompareResult _r,
        int256 _amount
    ) external returns (bytes memory);

    function decodeSwapRevertData(
        bytes memory _revert
    ) external returns (CompareResult _r, int256 _amount);
}
...
contract RevertDataTesting {
    ...
    constructor() {
        inner = IUniswapV3LiquidityBytesTest(
            address(new UniswapV3LiquidityBytesTest())
        );
    }
    ...
    function _testInner(CompareResult _r, int256 _amount) internal {
        try inner.revertSwapCallback(_r, _amount) {} catch (
            bytes memory reason
        ) {
            (CompareResult r, int256 amount) = inner.decodeSwapRevertData(
                reason
            );
...
contract UniswapV3LiquidityBytesTest is LiquiditySwapV3 {
    ...
    function decodeSwapRevertData(
        bytes memory _revert
    ) external pure returns (CompareResult _r, int256 _amount) {
